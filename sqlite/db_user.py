...
def create(db: Session, request: UserRequest) -> UserResponse:
    try:
        ...
        db.refresh(new_user)
        db.expunge(new_user)
        new_user_with_tasks = db.query(User).options(joinedload(User.tasks)).get(new_user.id)
        
        return UserResponse.from_orm(new_user_with_tasks)
       # return new_user
...
def login(db: Session, request: UserRequest) -> UserResponse:
    try:
        ...
        db.refresh(user)
        db.expunge(user)
        new_user_with_tasks = db.query(User).options(joinedload(User.tasks)).get(user.id)
